...
def check_if_notification_window(
    hwnd,
) -> Optional[Dict]:
    """检查窗口是否是通知弹窗"""
    ...
    try:
        ...
        # 明确仅支持 Chrome_WidgetWin（飞书通知弹窗窗口类）
...
class FeishuMonitor:
    ...
    def _win_event_callback(self, hWinEventHook, event, hwnd, idObject, idChild, dwEventThread, dwmsEventTime):
        ...
        try:
            ...
            # 快速检查窗口类名（明确仅支持 Chrome_WidgetWin）
            try:
