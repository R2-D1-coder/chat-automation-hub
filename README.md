...
```mermaid
flowchart TD
    START([开始]) --> CHECK[检查独立窗口]
    CHECK -->|未找到| ERROR1[提示打开独立窗口]
    CHECK -->|找到| WHITELIST{白名单校验}
    
    WHITELIST -->|不通过| ERROR2[WhitelistError]
    WHITELIST -->|通过| SAFETY{安全保险丝}
    
    SAFETY -->|未解除| ERROR3[SafetyError]
    SAFETY -->|已解除| LOOP[遍历目标群]
